...
contains
  ...
  !> Amplitude of the displacements of the selected vibrational mode, as
  !> animated in the view: the user amplitude scaled so that low-frequency
  !> modes move more, as a thermal displacement would. Requires a valid
  !> mode selection.
  module function scene_vib_amplitude(s) result(fac)
  ...
  end function scene_vib_amplitude

  !> Clear the selected vibrational mode and stop the animation.
  module subroutine scene_clear_vib_mode(s)
  ...
  end subroutine scene_clear_vib_mode
  ...
  module subroutine scene_build_lists(s)
    ...
    ! regenerate the vibration arrows (they depend on the display and on
  ...
  !> style the scene carries. This only arms the slot and forces a build if
  !> the mode, the phase or the arrow style changed; the list itself is
  !> regenerated by the build (vibarrow_refresh), once per build.
  module subroutine scene_show_transient_vibarrows(s,owner,tag)
    ...
    ! arm the slot (a new or retagged one has already forced a build)
    id = transient_slot(s,owner,tag,reptype_shapes,repflavor_shapes,found)
    if (id <= 0) return
    s%vibarrow_owner = owner
    s%vibarrow_tag = tag
    if (.not.found) return

    ! rebuild if what the list was generated from changed
    changed = (s%iqpt_selected /= s%vibarrow_geniqpt)
    changed = changed .or. (s%ifreq_selected /= s%vibarrow_genifreq)
    changed = changed .or. (abs(vibarrow_phase(s) - s%vibarrow_genphase) > 1d-10)
    changed = changed .or. (abs(s%vibarrow_length - s%vibarrow_genlength) > 1d-10)
    changed = changed .or. shape_differs(s%vibarrow,s%vibarrow_gen)
    if (changed) call transient_dirty(s)
  ...
  !> Regenerate the list of vibration displacement arrows in the transient
  !> shapes object that holds them, if there is one. Called by the scene
  !> build, before the draw lists are made, so the arrows are generated
  !> exactly once per build.
  subroutine vibarrow_refresh(s)
    use representations, only: reptype_shapes, vibration_arrow_shapes
    class(scene), intent(inout), target :: s

    integer :: i, id

    if (s%vibarrow_owner == 0) return

    ! find the slot (it is gone if the owner stopped arming it)
    id = 0
    do i = 1, s%nreptrans
       if (.not.s%reptrans(i)%isinit) cycle
       if (s%reptrans(i)%owner == s%vibarrow_owner .and. s%reptrans(i)%itag == s%vibarrow_tag .and.&
          s%reptrans(i)%type == reptype_shapes) then
          id = i
          exit
       end if
    end do
    ...
    ! generate the arrows straight into the slot's (grow-only) shape list
    s%vibarrow_geniqpt = s%iqpt_selected
    s%vibarrow_genifreq = s%ifreq_selected
    s%vibarrow_genphase = vibarrow_phase(s)
    ...
    call vibration_arrow_shapes(s%id,s%disp,s%iqpt_selected,s%ifreq_selected,&
       s%vibarrow_genphase,s%vibarrow_length,s%vibarrow,s%reptrans(id)%shapes%nshape,&
       s%reptrans(id)%shapes%shape)
    s%reptrans(id)%shapes%isel = 0

  end subroutine vibarrow_refresh

  !> Animation phase the vibration arrows are drawn at (units of pi/2):
  !> the manual phase when the animation is manual, reversed (+2) if the
  !> manual amplitude is negative; zero otherwise (the phase at which the
  !> automatic animation starts).
  function vibarrow_phase(s) result(phase)
    class(scene), intent(in) :: s
    real*8 :: phase
    ...
    if (s%animation == 1) then
       phase = s%anim_phase
       if (s%anim_amplitude < 0d0) phase = phase + 2d0
    end if

  end function vibarrow_phase
  ...
  subroutine sphere_pack(col,x,r,rgba,border,bcol,xdelta,ridx,occ,occempty,piecum,piergb,rim)
    ...
    logical, intent(in), optional :: rim ! translucent volume: opacity growing toward the rim
  ...
  subroutine cone_model(x1,x2,rad,model)
    ...
    real(c_float) :: xmid(3), xdif(3), up(3), crs(3), blen, ca, sa, axis(3), temp(3)
    ...
    if (dot_product(crs,crs) > 1e-14_c_float) then
       ! xdif and up are unit vectors: cos is xdif(3), sin is |up x xdif|
       ca = xdif(3)
       sa = norm2(crs)
       axis = crs / sa
