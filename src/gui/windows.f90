...
module windows
  ...
  type voids_state
     ...
     real*8, allocatable :: iso_f(:,:,:) ! the promolecular density on that grid, kept so that
                                         ! moving the isovalue costs only the flood fill (dropped
                                         ! by drop_caches, and then sampled again on demand)
