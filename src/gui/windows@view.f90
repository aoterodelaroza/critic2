...
contains
  ...
  module subroutine draw_editrep(w)
    use scenes, only: representation, reptype_atoms, reptype_bonds, reptype_unitcell,&
       reptype_labels
    use windows, only: nwin, win, wintype_view
  ...
  module function draw_editrep_bonds(w,ttshown) result(changed)
    ...
    call iw_text("Filter",highlight=.true.)
    call iw_text("(?)",sameline=.true.)
    call iw_tooltip("Show the bond if the filter expression evaluates to non-zero (true) at the bond position. "&
       &"Structural variables are very useful for filters. Examples:"//newline//&
       "- '@x < 3' = all bonds with x lower than 3"//newline//&
       "- 'log($0) > 1' = log of the promolecular density higher than 1"//newline//&
       "- 'abs(@x) < 2 && abs(@y) < 2 && abs(@z) < 2' = bonds in the (-2,2) box"//newline//&
       "Click on the Help button for more info.")
    if (iw_button("Help",sameline=.true.)) then
       str3 = "https://aoterodelaroza.github.io/critic2/manual/arithmetics" // c_null_char
       call openLink(c_loc(str3))
    ...
    if (len_trim(w%rep%filter) == 0) w%rep%goodfilter = .true.
    call iw_tooltip("Apply this filter to the bonds in the system. Bonds are represented if non-zero.",&
       ttshown)
    if (iw_button("Clear",sameline=.true.)) then
       w%rep%filter = ""
       w%rep%goodfilter = .true.
    ...
    if (.not.w%rep%goodfilter) &
       call iw_text("Error in the filter expression.",danger=.true.)
    changed = changed .or. iw_radiobutton("Bond center",int=w%rep%bond_filter_type,intval=0_c_int)
    call iw_tooltip("Draw a bond if the filter is non-zero at the bond center",ttshown)
    changed = changed .or. iw_radiobutton("Either end",int=w%rep%bond_filter_type,intval=1_c_int,sameline=.true.)
    call iw_tooltip("Draw a bond if the filter is non-zero at either end",ttshown)
    changed = changed .or. iw_radiobutton("Both ends",int=w%rep%bond_filter_type,intval=2_c_int,sameline=.true.)
    call iw_tooltip("Draw a bond if the filter is non-zero at both ends",ttshown)
    ...
    if (.not.sys(isys)%c%ismolecule) then
       ...
       if (sys(isys)%c%ismol3d) then
          str2 = "Show connected molecules" // c_null_char
          changed = changed .or. igCheckbox(c_loc(str2),w%rep%onemotif)
          call iw_tooltip("Translate bonds to display whole molecules",ttshown)
       end if

       ! checkbox for border
       str2 = "Show bonds at cell edges" // c_null_char
       changed = changed .or. igCheckbox(c_loc(str2),w%rep%border)
       call iw_tooltip("Display bonds near the unit cell edges",ttshown)
    end if
