...
!
! The subroutines from space_group.f90 are
!
...
contains
  ...
  module subroutine sup_spacegroup(tau,ityp,extfor,if_pos,space_group_number,not_eq,&
    ...
    if (allocated(tautot)) deallocate(tautot)
    if (allocated(extfortot)) deallocate(extfortot)
    if (allocated(ityptot)) deallocate(ityptot)
    if (allocated(if_postot)) deallocate(if_postot)
  ...
  ! The following subroutines are from the space_group.f90 module of Quantum ESPRESSO.
  !
  ! Copyright (C) 2014 Federico Zadra
  ! This file is distributed under the terms of the
  ! GNU General Public License. See the file `License'
