...
module tricks
  ...
  ! private :: trick_check_shortest()
  ! private :: trick_test_environment()
...
contains
  ...
  subroutine trick(line0)
    ...
    else if (equal(word,'energy')) then
...
999 continue
  ...
  subroutine trick_force_constants(line0)
    ...
    integer, parameter :: imode_calcq = 2
    integer :: imode
    ...
    do while (.true.)
       ...
       elseif (equal(lword,'vasp') .or. equal(lword,'fhiaims')) then
          ...
          stop 1
       elseif (equal(lword,'print')) then
          imode = imode_print
       elseif (equal(lword,'q')) then
          imode = imode_calcq
          ok = eval_next(q(1),line0,lp)
          ok = ok .and. eval_next(q(2),line0,lp)
          ok = ok .and. eval_next(q(3),line0,lp)
          if (.not.ok) &
             call ferror('trick_force_constants','Error reading q',faterr,line0,syntax=.true.)
    ...
    if (.not.haveformat) &
       call ferror('trick_force_constants','A format is required',faterr)
    if (imode == imode_unk) &
       call ferror('trick_force_constants','An operation mode is required',faterr)
    ...
    elseif (imode == imode_calcq) then
       !!! calculate frequencies at a q-point q !!!
       call test(q)
    end if
...
999 continue
  ...
  contains
    ...
    subroutine test(q)
      ...
      ! sqrt(4.3597447222071e-18) / 5.29177210903e-11 / sqrt(1.66053906660e-27) / 1e12 / 2 / pi
      ! ans = 154.1079276131967
      real*8, parameter :: factor = 154.1079276131967d0
      ! sqrt(4.3597447222071e-18) / 5.29177210903e-11 / sqrt(1.66053906660e-27) /  2 / pi / c / 100
      ! ans = 5140.487143715827
      ! real*8, parameter :: factor = 5140.487143715827d0

      ! good! only for q = 0 0 0
      ...
      allocate(dm(sy%c%ncel*3, sy%c%ncel*3))
      allocate(rused(sy%c%ncel),mult(sy%c%ncel))
      dm = 0d0
      do i = 1, sy%c%ncel
         rused = huge(1d0)
         mult = 0
         call sy%c%list_near_atoms(sy%c%atcel(i)%x,icrd_crys,.true.,nat,nida,dist=dist,lvec=lvec,&
            up2d=min(disteps,maxdisteps+1d-4))
         do jj = 1, nat
            j = nida(jj)
            if (dist(jj) <= rused(j) + 1d-5) then
               x1 = sy%c%atcel(i)%x
  ...
  subroutine trick_energy(line0)
    ...
    ! actually exercised by the finite-difference check
    do i = 1, nat
       do k = 1, 3
          sy%c%atcel(i)%r(k) = sy%c%atcel(i)%r(k) + 0.15d0*sin(1.7d0*i + 2.3d0*k + 0.9d0)
       end do
    end do
    ...
    gmax = maxval(abs(gfd - grad))
    write (uout,'("  max |grad_analytic - grad_fd| (hartree/bohr): ",A)') string(gmax,'e',decimal=6)

    ! finite-difference stress (crystals only)
    if (.not.sy%c%ismolecule) then
       m_save = sy%c%m_x2c
       omega0 = sy%c%omega
       do a = 1, 3
          do b = 1, 3
             ! deformation gradient F = I + h e_a (x) e_b
             do i = 1, nat
  ...
  subroutine trick_md(line0)
    ...
    do istep = 1, nstep
       call md%step(sy%c)
       etot = md%ekin + md%epot
       drift = max(drift,abs(etot-e0))
    end do
    write (uout,'("  NVE steps:                 ",A)') string(nstep)
    write (uout,'("  initial total energy (Ha): ",A)') string(e0,'e',decimal=8)
    write (uout,'("  max energy drift (Ha):     ",A)') string(drift,'e',decimal=8)
    if (abs(e0) > 1d-12) &
    ...
    do istep = 1, nstep
       call md%step(sy%c)
       if (istep > nstep/2) tsum = tsum + md%temperature_now()
    end do
    write (uout,'("  target temperature (K):    ",A)') string(300d0,'f',decimal=2)
    ...
    do i = 1, md%nat
       ...
       md%r(3,i) = md%r(3,i) + 0.1d0*sin(0.7d0*i+2.4d0)
    end do
    nstep = 2000
    do istep = 1, nstep
       call md%step(sy%c)
