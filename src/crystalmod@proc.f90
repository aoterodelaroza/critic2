...
999 continue
  ...
  module subroutine writegrid_xsf(c,g,file,onlyheader)
    ...
    do j = 1, c%ncel
       write (lu,'(A,X,1p,3(E22.14,X))') string(c%spc(c%atcel(j)%is)%z), c%atcel(j)%r * bohrtoa
    end do
    if (.not.onlyheader) then
       ...
       write (lu,'(1p,5(1x,e22.14))') (((g(mod(ix,n(1))+1,mod(iy,n(2))+1,mod(iz,n(3))+1),ix=0,n(1)),iy=0,n(2)),iz=0,n(3))
