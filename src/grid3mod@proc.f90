...
999 continue
  ...
  module subroutine fft(fnew,fold,iff)
    ...
    integer :: n(3), i1, i2, i3, ntot, igfft, i
    ...
    fnew%isinit = .true.
    fnew%mode = mode_default
    ntot = n(1) * n(2) * n(3)

    ! reciprocal lattice vectors
    bvec(:,1) = cross(fold%x2c(:,3),fold%x2c(:,2))
    bvec(:,2) = cross(fold%x2c(:,1),fold%x2c(:,3))
    bvec(:,3) = cross(fold%x2c(:,2),fold%x2c(:,1))
    bvec = 2d0 * pi / abs(det3(fold%x2c)) * bvec
  ...
  subroutine init_smr(f)
   ...
   subroutine crystalmod_promolecular(cptr,x0,icrd,f,fp,fpp,nder,zpsp,fr)
     use iso_c_binding, only: c_f_pointer
