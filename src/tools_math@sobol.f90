! Sobol sequence generators. Slightly modified from:
! Original FORTRAN77 version by Bennett Fox; FORTRAN90 version by John Burkardt.
! Distributed under the GNU LGPL license.
!
! From https://people.sc.fsu.edu/~jburkardt/f_src/sobol/sobol.html
!
...
!
! The code is an adaptation of the INSOBL and GOSOBL routines in ACM
! TOMS Algorithm 647 and ACM TOMS Algorithm 659. The original code can
! only compute the "next" element of the sequence. The revised code
! allows the user to specify the index of any desired element.
!
! At the request of a user, the library now contains two versions of the
! algorithm, one using the default integer and real precision, and one
! using the higher precision obtainable by requesting "(KIND=8)". The
! two sets of code are distinguished by names that begin with I4_ and
! I8_. I am aware that using the KIND attribute with an explicit value
! is not the correct way of asking for higher precision, but it's
! natural, and the correct way looks a little too awkward and obscure to
! me today. I am aware that many variables in the higher precision code
! need not themselves be calculated in higher precision. I'd just like
! to see if and how the higher precision code runs at all.
!
...
! dimension of 1111. These changes have been implemented in the C++ and
! FORTRAN90 versions of the program.
!
! The original, true, correct versions of ACM TOMS Algorithm 647 and ACM
! TOMS Algorithm 659 are available in the TOMS subdirectory of the
...
! Copyright (c) 2022 Alberto Otero de la Roza <aoterodelaroza@gmail.com>,
...
contains
  ...
  module subroutine sobol_sequence(n,x,seed)
    ...
    integer*8, save :: seed_save = 0

    if (present(seed)) seed_save = seed
    call i8_sobol(int(n,8),seed_save,x)
