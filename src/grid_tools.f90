...
contains
  ...
  !> Read the UNK generated by QE for Wannier.
  subroutine grid_read_unk(file,f,omega,saveunk)
    use tools_io, only: fopen_read, getline_raw, lgetword, equal, ferror, faterr, &
    ...
    type(field), intent(inout) :: f
    real*8, intent(in) :: omega
    logical, intent(in) :: saveunk

    integer :: luc, luw
    character(len=:), allocatable :: fname
    integer :: ispin, iz, ikk, ibnd, ik, nbnd, jbnd
    integer :: n(3), ik1, ik2, ik3, imax(3)
    real*8 :: fspin
    integer :: i, j, k
    complex*16, allocatable :: raux(:,:,:)
    integer :: nk1, nk2, nk3, nk, nspin

    ! get the information from the unk help file
    luc = fopen_read(file)
    read (luc,*) nk1, nk2, nk3, nspin, nbnd, n
    nk = nk1 * nk2 * nk3
    if (allocated(f%wan_kpt)) deallocate(f%wan_kpt)
    allocate(f%wan_kpt(3,nk))
    do i = 1, nk
       read (luc,*) f%wan_kpt(:,i)
    end do
    call fclose(luc)
    ...
    if (nspin == 1) then
       ...
       fspin = 1d0
       call ferror('grid_read_unk','nspin = 2 not implemented yet',faterr)
    end if
    ...
    do jbnd = 1, nbnd
       do ispin = 1, nspin
          call get_qe_psink(jbnd,ispin,n,nk1,nk2,nk3,f%wan_kpt,.true.,saveunk,raux)
          ! density contribution
          do i = 1, nk1
             do j = 1, nk2
                do k = 1, nk3
                   f%f = f%f + real(raux((i-1)*n(1)+1:i*n(1),(j-1)*n(2)+1:j*n(2),(k-1)*n(3)+1:k*n(3)),8)**2 + &
                      aimag(raux((i-1)*n(1)+1:i*n(1),(j-1)*n(2)+1:j*n(2),(k-1)*n(3)+1:k*n(3)))**2
                end do
             end do
          end do
       end do
  ...
  subroutine get_qe_psink(ibnd,ispin,n,nk1,nk2,nk3,kpt,usewnk,saveunk,fout)
    ...
    character(len=:), allocatable :: fname, wname
    complex*16 :: tnorm
    ...
    do ik = 1, nk
       ...
       wname = "WNK" // string(ik) // "." // string(ibnd) // "." // string(ispin)
       inquire(file=wname,exist=ok)

       if (usewnk .and. .not.ok .or..not.usewnk) then
          ! open file for reading
          luc = fopen_read(fname,form="unformatted")

          ! read the header
          read(luc) naux, ikk, nbnd
          if (any(naux /= n) .or. ikk /= ik) &
             call ferror('grid_read_unk','error in read unk file',faterr)
          ...
          call fclose(luc)
          if (usewnk) then
             luc = fopen_write(wname,form="unformatted")
             write (luc) raux
             call fclose(luc)
          end if
       else
          ! open file for reading
          luc = fopen_read(wname,form="unformatted")
          read(luc) raux
          do ikk = 1, nk
             ik1 = nint(kpt(1,ikk) * nk1)
             ik2 = nint(kpt(2,ikk) * nk2)
       ...
       ! clean up the unks
       if (usewnk.and..not.saveunk) then
          inquire(file=fname,exist=ok)
          if (ok) call unlink(fname)
       end if
    end do
