...
contains
  ...
  subroutine grid_read_unk(file,f,omega)
    ...
    integer :: ispin, iz, ikk, ibnd, ik, nbnd
    integer :: n(3), ik1, ik2, ik3, imax(3)
    real*8 :: fspin
    integer :: i, j, k
    complex*16, allocatable :: raux(:,:,:), raux2(:,:,:), raux3(:,:,:)
    complex*16 :: tnorm
    ...
    ! allocate arrays
    fname = "UNK" // string(1,5,pad0=.true.) // "." // string(1)
    ...
    allocate(f%f(f%n(1),f%n(2),f%n(3)))
    if (allocated(f%fwan)) deallocate(f%fwan)
    allocate(f%fwan(f%n(1)*nk1,f%n(2)*nk2,f%n(3)*nk3,nbnd,nspin))
    allocate(raux(n(1),n(2),n(3)))
    allocate(raux3(nk1*n(1),nk2*n(2),nk3*n(3)))
    ...
    do ik = 1, nk
       ...
       call cfftnd(3,f%nwan*n,1,raux2)
       ...
       do ispin = 1, nspin
          ...
          read(luc) n, ikk, nbnd
          if (.not.(ispin == 1 .and. ik == 1)) then
             if (any(n /= f%n) .or. ikk /= ik .or. nbnd /= size(f%fwan,4)) &
          ...
          do ibnd = 1, nbnd
             ...
             do ikk = 1, nk
                ...
                raux3(ik1*n(1)+1:(ik1+1)*n(1),ik2*n(2)+1:(ik2+1)*n(2),ik3*n(3)+1:(ik3+1)*n(3)) = &
                   raux2(ik1*n(1)+1:(ik1+1)*n(1),ik2*n(2)+1:(ik2+1)*n(2),ik3*n(3)+1:(ik3+1)*n(3)) * &
                   raux
             end do
             f%fwan(:,:,:,ibnd,ispin) = f%fwan(:,:,:,ibnd,ispin) + raux3
          end do

          call fclose(luc)
       end do
    end do
    deallocate(raux,raux2,raux3)
    
    ! normalize and density contribution
    do ispin = 1, nspin
       do ibnd = 1, nbnd
          imax = maxloc(abs(f%fwan(:,:,:,ibnd,ispin)))
          tnorm = f%fwan(imax(1),imax(2),imax(3),ibnd,ispin)
          tnorm = tnorm / abs(tnorm) * nk
          f%fwan(:,:,:,ibnd,ispin) = f%fwan(:,:,:,ibnd,ispin) / tnorm
