! Copyright (c) 2013 Alberto Otero de la Roza <alberto@fluor.quimica.uniovi.es>,
...
contains
  ...
  recursive subroutine get_node(self,key,val)
    ...
    if (trim(self%key) == trim(key)) then
  ...
  function keys_hash(self) result (nkeys)
    ...
    nkeys = 0
    do i = 1, self%len
       if (len_trim(self%root(i)%key) > 0) nkeys = nkeys + 1
       cur => self%root(i)%next
       do while(associated(cur))
          if (len_trim(cur%key) > 0) nkeys = nkeys + 1
          cur => cur%next
       end do
    end do
  ...
  function getkey_hash(self,ikey) result(key)
    ...
    do i = 1, self%len
       ...
       if (len_trim(self%root(i)%key) > 0) then
          nkeys = nkeys + 1
          if (nkeys == ikey) then
             key = trim(self%root(i)%key)
             return
          end if
       endif
       cur => self%root(i)%next
       do while(associated(cur))
          if (len_trim(cur%key) > 0) then
             nkeys = nkeys + 1
             if (nkeys == ikey) then
                key = trim(cur%key)
                return
